...
(defun marong/insert-semicolon-at-the-end-of-this-line ()
  (interactive)
  (save-excursion
    (end-of-line)
    (insert ";")))

(defun marong/delete-semicolon-at-the-end-of-this-line ()
  (interactive)
  (save-excursion
    (end-of-line)
    (if (looking-back ";")
...
(defun marong/insert-comma-at-the-end-of-this-line ()
  (interactive)
  (save-excursion
    (end-of-line)
    (insert ",")))

(defun marong/delete-comma-at-the-end-of-this-line ()
  (interactive)
  (save-excursion
    (end-of-line)
    (if (looking-back ",")
...
(defun marong/indent-region(numSpaces)
  (progn
...
(defun marong/tab-region (N)
  (interactive "p")
  (if (use-region-p)
      (marong/indent-region 2)               ; region was selected, call indent-region
    (insert "    ")                   ; else insert four spaces as expected
    ))

(defun marong/untab-region (N)
  (interactive "p")
  (marong/indent-region -2))

(defun marong/hack-tab-key ()
  (interactive)
  (local-set-key (kbd "<tab>") 'marong/tab-region)
  (local-set-key (kbd "<S-tab>") 'marong/untab-region)
  )

;; I'm don't like this settings too much.
;; (add-hook 'prog-mode-hook 'marong/hack-tab-key)
...
(defun marong/helm-hotspots ()
  "helm interface to my hotspots, which includes my locations,
org-files and bookmarks"
  (interactive)
  (helm :buffer "*helm: utities*"
        :sources `(,(marong//hotspots-sources))))

(defun marong//hotspots-sources ()
  ...
  `((name . "Mail and News")
    (candidates . (("Calendar" . (lambda ()  (browse-url "https://www.google.com/calendar/render")))
                   ...
                   ("Run current flie" . (lambda () (marong/run-current-file)))
...
(defun marong/browser-refresh--chrome-applescript ()
  (interactive)
  (do-applescript
   (format
    "
...
(defun marong/open-file-with-projectile-or-counsel-git ()
  (interactive)
  (if (or (marong/vcs-project-root)
...
(defun marong/growl-notification (title message &optional sticky)
...
(defun marong/growl-timer (minutes message)
  ...
  (run-at-time (* (string-to-number minutes) 60)
               ...
               (lambda (minute message)
                 (marong/growl-notification "Emacs Reminder" message t))
               minutes
               message))

(defun marong/goto-match-paren (arg)
...
(defun marong/hidden-dos-eol ()
  "Do not show ^M in files containing mixed UNIX and DOS line endings."
  (interactive)
  (setq buffer-display-table (make-display-table))
  (aset buffer-display-table ?\^M []))

(defun marong/remove-dos-eol ()
  "Replace DOS eolns CR LF with Unix eolns CR"
  (interactive)
  (goto-char (point-min))
  (while (search-forward "\r" nil t) (replace-match "")))

(defun marong/insert-chrome-current-tab-url()
  "Get the URL of the active tab of the first window"
  (interactive)
  (insert (marong/retrieve-chrome-current-tab-url)))

(defun marong/retrieve-chrome-current-tab-url()
  "Get the URL of the active tab of the first window"
  (interactive)
...
(defun marong/single-lines-only ()
...
(defun marong/named-term (name)
  (interactive "sName: ")
  (ansi-term "/bin/zsh" name))


(defun marong/ash-term-hooks ()
...
(defun marong/word-count-for-chinese ()
  "「較精確地」統計中/日/英文字數。
- 文章中的註解不算在字數內。
...
(defun marong/evil-quick-replace (beg end )
  (interactive "r")
...
(defun marong/iterm-shell-command (command &optional prefix)
  "cd to `default-directory' then run COMMAND in iTerm.
With PREFIX, cd to project root."
  (interactive (list (read-shell-command
                      "iTerm Shell Command: ")
                     current-prefix-arg))
  (let* ((dir (if prefix (marong/vcs-project-root)
                default-directory))
...
(defun marong/my-mc-mark-next-like-this ()
  (interactive)
  (if (region-active-p)
...
(defun my-open-file-in-external-app (file)
  ...
  (interactive)
  (let ((default-directory (marong/vcs-project-root))
        (file-path file))
    (if file-path
        (cond
...
(defun marong/magit-visit-pull-request ()
...
(defun marong/search-in-fireball ()
  (interactive)
  (helm-do-ag (expand-file-name "~/Github/fireball/")))


(defun marong/show-current-buffer-major-mode ()
  (interactive)
  (describe-variable 'major-mode))

(defun marong/counsel-imenu ()
  (interactive)
  (counsel-imenu)
  (evil-set-jump))
